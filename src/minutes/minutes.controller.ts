...
export class MinutesController {
    ...
    async getById(@Param() param): Promise<Minute> {
        ...
        if (association === undefined) {
            throw new HttpException(`Could not find a minute with the id ${+param.id}`, HttpStatus.NOT_FOUND);
        } 
        else {
            return association;
        }
    ...
    async create(@Body() input : MinuteInput): Promise<Minute> {
        return await this.service.create(input.date, input.content, input.idVoters, input.idAssociation);
    }
    ...
    async update(@Param() param, @Body() input:MinuteUpdate) : Promise<void>{
        if(await this.service.update(+param.id, input.date, input.content, input.idVoters, input.idAssociation) === undefined) throw new HttpException(`Could not find a Minute with the id ${+param.id}`, HttpStatus.NOT_FOUND);
    }
